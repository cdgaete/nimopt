...
def test_an_unknown_method_raises():
    call_error(
        "method of piecewise 'curve' is one of ('incremental', 'tangent', "
        "'auto'); got 'sos2'",
        method="sos2",
    )
...
def auto_model(yp_values, sign=">=", active=False, shift=0.0):
    G, B, xp, yp, m = one_generator(yp_values)
    p = m.var("p", (G,))
    c = m.var("c", (G,), lower=-np.inf)
    on = m.var("on", (G,), upper=1.0, integer=True)
    s = m.var("s", ())
    m.constraint("balance", no.Sum(G, p[G]) + s == 15.0)
    m.piecewise(
        "curve",
        ...
        sign=sign,
        method="auto",
        active=on[G] if active else None,
    )
    m.set_objective(no.Sum(G, c[G]) + 1.5 * s)
    return m


def test_auto_uses_tangent_for_a_convex_curve_under_greater_equal():
    m = auto_model([0.0, 10.0, 30.0])
    d = m.piecewise_declarations["curve"]
    assert d.method == "auto"
    assert d.formulation == "tangent"
    assert "curve_tangent" in m.constraints
    assert "curve_fill" not in m.variables
    assert m.solve().objective == pytest.approx(17.5)


@pytest.mark.parametrize(
    "changes",
    [
        {"yp_values": [0.0, 20.0, 30.0]},
        {"yp_values": [0.0, 10.0, 30.0], "sign": "=="},
        {"yp_values": [0.0, 10.0, 30.0], "active": True},
        {"yp_values": [0.0, 10.0, 30.0], "shift": 1.0},
    ],
    ids=["not convex", "equality", "active", "constant"],
)
def test_auto_uses_incremental_where_tangent_does_not_apply(changes):
    m = auto_model(**changes)
    assert m.piecewise_declarations["curve"].formulation == "incremental"
    assert "curve_fill" in m.variables
    assert "curve_tangent" not in m.constraints


def test_auto_and_the_method_it_uses_reach_the_same_optimum():
    for values in ([0.0, 10.0, 30.0], [0.0, 20.0, 30.0]):
        m = auto_model(values)
        method = m.piecewise_declarations["curve"].formulation
        G, B, xp, yp, twin = one_generator(values)
        p = twin.var("p", (G,))
        c = twin.var("c", (G,), lower=-np.inf)
        s = twin.var("s", ())
        twin.constraint("balance", no.Sum(G, p[G]) + s == 15.0)
        twin.piecewise("curve", p[G], xp[G, B], c[G], yp[G, B], ">=", method)
        twin.set_objective(no.Sum(G, c[G]) + 1.5 * s)
        assert m.solve().objective == pytest.approx(twin.solve().objective)


def test_auto_raises_for_x_points_that_are_not_monotonic():
    G = no.Set("G", np.array(["a"]))
    B = no.Set("B", np.array(["b0", "b1", "b2"]))
    xp = no.Param.from_dense("xp", (G, B), [[0.0, 10.0, 10.0]])
    yp = no.Param.from_dense("yp", (G, B), [[0.0, 10.0, 30.0]])
    m = no.Model("repeat")
    p = m.var("p", (G,))
    c = m.var("c", (G,))
    with pytest.raises(ValueError, match="not strictly monotonic"):
        m.piecewise("curve", p[G], xp[G, B], c[G], yp[G, B], ">=", "auto")


def test_an_explicit_method_is_its_own_formulation():
    m = cost_model()
    assert m.piecewise_declarations["curve"].formulation == "incremental"


def test_a_definition_reserves_the_names_of_both_methods_for_auto():
    d = no.Definition("reserve")
    G = d.set("G")
    B = d.set("B")
    xp = d.param("xp", (G, B))
    p = d.var("p", (G,))
    c = d.var("c", (G,))
    d.piecewise("curve", p[G], xp[G, B], c[G], xp[G, B], ">=", "auto")
    for name in ("curve_fill", "curve_tangent"):
        with pytest.raises(ValueError, match="is generated by piecewise 'curve'"):
            d.var(name, (G,))
