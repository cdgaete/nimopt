...
def test_a_term_spells_its_variable_at_its_sets():
    G, T, gen, *_ = fleet()
    assert spell(gen[G, T]) == "gen[G, T]"


def test_a_coefficient_spells_with_its_brackets():
    G, T, gen, cost, *_ = fleet()
    assert spell(cost[G, T] * gen[G, T]) == "cost[G, T] * gen[G, T]"
...
def test_a_sum_wraps_each_term_it_reduces():
    ...
    assert (
        spell(Sum(G, cost[G, T] * gen[G, T] + gen[G, T]))
        == "Sum(G, cost[G, T] * gen[G, T]) + Sum(G, gen[G, T])"
    )
    assert spell(Sum(G, T, gen[G, T])) == "Sum(G, T, gen[G, T])"


def test_a_lag_and_a_cyclic_lag_spell_as_typed():
    G, T, gen, *_ = fleet()
    assert spell(gen[G, T - 1]) == "gen[G, T - 1]"
    assert spell(gen[G, T + 2]) == "gen[G, T + 2]"
    assert spell(gen[G, T.cyclic - 1]) == "gen[G, T.cyclic - 1]"


def test_a_fixed_member_spells_as_its_label():
    G, T, gen, cost, *_ = fleet()
    assert spell(cost[G, "t0"] * gen[G, "t0"]) == "cost[G, 't0'] * gen[G, 't0']"
...
def test_a_numeric_label_spells_bare_whatever_its_type():
    ...
    m = Model("m")
    x = m.var("x", (K,))
    assert spell(x[0]) == "x[0]"
    assert spell(x[np.int64(2)]) == "x[2]"


def test_a_derived_coefficient_spells_as_its_tree():
    G, T, gen, cost, eta, _ = fleet()
    unit = cost[G, T] / eta[G, T]
    assert spell(unit * gen[G, T]) == "(cost[G, T] / eta[G, T]) * gen[G, T]"
    assert spell(2 * unit * gen[G, T]) == "((cost[G, T] / eta[G, T]) * 2) * gen[G, T]"
    assert spell((-cost[G, T]) * gen[G, T]) == "(-cost[G, T]) * gen[G, T]"
    assert spell((2 - cost[G, T]) * gen[G, T]) == "(2 - cost[G, T]) * gen[G, T]"
    assert spell((cost[G, T] ** 2) * gen[G, T]) == "(cost[G, T] ** 2) * gen[G, T]"


def test_a_derived_coefficient_read_at_its_sets_spells_once():
    G, T, gen, cost, eta, _ = fleet()
    unit = cost[G, T] / eta[G, T]
    assert spell(unit[G, T] * gen[G, T]) == "(cost[G, T] / eta[G, T]) * gen[G, T]"
    assert (
        spell(unit[G, "t0"] * gen[G, "t0"])
...
def test_a_scale_leads_and_parenthesises_a_product():
    G, T, gen, cost, *_ = fleet()
    assert spell(2 * gen[G, T]) == "2 * gen[G, T]"
    assert spell(0.5 * gen[G, T]) == "0.5 * gen[G, T]"
    assert spell(2 * (cost[G, T] * gen[G, T])) == "2 * (cost[G, T] * gen[G, T])"
    assert (
        spell(2 * Sum(G, cost[G, T] * gen[G, T]))
...
def test_a_negative_term_leads_with_a_minus_and_follows_with_one():
    ...
    assert spell(-2 * (cost[G, T] * gen[G, T])) == "-2 * (cost[G, T] * gen[G, T])"
    assert (
        spell(gen[G, T] - cost[G, T] * gen[G, T])
        == "gen[G, T] - cost[G, T] * gen[G, T]"
    )
    assert spell(gen[G, T] - 2 * gen[G, T]) == "gen[G, T] - 2 * gen[G, T]"


def test_a_constant_stands_last():
    G, T, gen, *_ = fleet()
    assert spell(gen[G, T] + 1) == "gen[G, T] + 1"
    assert spell(gen[G, T] - 1.5) == "gen[G, T] - 1.5"


def test_a_relation_spells_its_sense_and_right_hand_side():
    G, T, gen, cost, eta, load = fleet()
    assert spell(Sum(G, gen[G, T]) == load[T]) == "Sum(G, gen[G, T]) == load[T]"
    assert spell(gen[G, T] <= 5) == "gen[G, T] <= 5"
    assert spell(gen[G, T] >= cost[G, T]) == "gen[G, T] >= cost[G, T]"
    assert spell(gen[G, T] >= cost[G, T] - 1) == "gen[G, T] >= (cost[G, T] - 1)"
    assert spell(gen[G, T] <= gen[G, T - 1]) == "gen[G, T] - gen[G, T - 1] <= 0"


def test_a_sum_condition_spells_a_parameter_or_a_tuple_of_sets():
    G, T, gen, cost, *_ = fleet()
    assert spell(Sum(G, gen[G, T], where=cost)) == "Sum(G, gen[G, T], where=cost)"
    assert spell(Sum(G, gen[G, T], where=(G, T))) == "Sum(G, gen[G, T], where=(G, T))"
    assert spell(Sum(T, gen[G, T], where=(T,))) == "Sum(T, gen[G, T], where=(T,))"
...
def test_a_condition_with_no_name_is_refused():
    ...
    with pytest.raises(ValueError, match="no name"):
        spell(Sum(G, gen[G, T], where=arcs))


def test_a_coefficient_spells_on_its_own():
    G, T, _, cost, eta, _ = fleet()
    assert spell(cost[G, T]) == "cost[G, T]"
    assert spell(cost[G, T] / eta[G, T]) == "(cost[G, T] / eta[G, T])"
...
def test_a_spelling_is_a_fixed_point_of_reading(text):
    assert spell(read(text, symbols(declared()))) == text
...
def test_a_hand_edit_reads_as_the_same_edit_in_python(edited, canonical):
    assert spell(read(edited, symbols(declared()))) == canonical
...
def test_every_worked_model_spells_to_a_fixed_point(name):
    ...
    for relation, _, _ in d.constraints.values():
        text = spell(relation)
        assert spell(read(text, held)) == text
    text = spell(d.objective)
    assert spell(read(text, held)) == text
