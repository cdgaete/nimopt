...
from nimopt.names import check_addressable
...
class Piecewise:
    ...
    def __init__(
        ...
        self.name = str(name)
        check_addressable(self.name, "piecewise")
...
def taken(names: Mapping[str, tuple[str, ...]], declared: Container[str]) -> list[str]:
    """Return the names of `names`, by kind, that `declared` contains."""
    return [name for kind in KINDS for name in names[kind] if name in declared]


def check_not_generated(
    name: str, what: str, declarations: Mapping[str, Piecewise]
) -> None:
    """Raise ValueError where a declaration of `declarations` generates `name`."""
    for held, declaration in declarations.items():
        if any(name in generated for generated in declaration.names().values()):
            raise ValueError(
                f"{what} {name!r} is generated by piecewise {held!r}; declare "
                f"another name"
            )
