...
class Definition:
    ...
    def _fresh(self, name: str, registry: Mapping[str, Any], what: str) -> str:
        ...
        check_not_generated(name, what, self.piecewise_declarations)
        if registry is self.constraints:
            return name
        check_addressable(name, what)
        held = (
            ("set", self.sets),
            ("alias", self.aliases),
        ...
        check_one_kind(
            name, what, [(kind, other) for kind, other in held if other is not registry]
        )
        return name
    ...
    def piecewise(
        ...
        held = taken(names, {n for registry in registries for n in registry})
        if held:
            raise ValueError(
                f"piecewise {name!r} generates {held}, already declared in "
